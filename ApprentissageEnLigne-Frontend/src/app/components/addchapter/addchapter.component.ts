...
export class AddchapterComponent implements OnInit {
  name: string = "";
  courseslist: Observable<Course[]> | undefined;
  chapter = new Chapter();
  coursenames: Observable<Course[]> | undefined;
  loggedUser = '';

  constructor(private _router: Router, private _service: ProfessorService, private route: ActivatedRoute) { }

  ngOnInit() {
    this.name = this.route.snapshot.params['coursname'];
    console.log(this.name);
    this.chapter.coursename = this.name;
    this.loggedUser = JSON.stringify(sessionStorage.getItem('loggedUser') || '{}');
    this.loggedUser = this.loggedUser.replace(/"/g, '');
    this.name = this.route.snapshot.params['nom'];
